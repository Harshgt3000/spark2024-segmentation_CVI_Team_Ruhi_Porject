...
input_dir = "predictions_fix"
output_dir = "submission_fix2"
...
for i, f in enumerate(files):
    ...
    if len(img.shape) == 2:
        ...
        rgb[img == 1] = [255, 0, 0]   # Body - Red
        rgb[img == 2] = [0, 0, 255]   # Panels - Blue
        img = rgb
    
    # Ensure shape is (1024, 1024, 3)
    if img.shape != (1024, 1024, 3):
        print(f"Warning: {f} has shape {img.shape}")
...
print(f"Keys: {list(sample.keys())}, Shape: {sample['data'].shape}, Dtype: {sample['data'].dtype}")

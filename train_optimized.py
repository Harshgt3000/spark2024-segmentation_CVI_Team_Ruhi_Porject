#!/usr/bin/env python3
"""
Optimized training with memory-efficient evaluation based on expert recommendations:
1. torch.inference_mode() instead of no_grad (faster, less memory)
2. FP16 inference with autocast
3. Streaming mIoU via confusion matrix (O(num_classes²) memory)
4. Proper gc.collect() + empty_cache() placement
5. Batch size 1 for validation
6. Immediate tensor deletion
"""
import os
import sys
import argparse
...
def compute_miou_streaming(model, val_loader, device, num_classes=3):
    """
    Memory-efficient streaming mIoU using confusion matrix.
    Only stores O(num_classes²) instead of O(samples × H × W)
    """
    model.eval()
    
    # Confusion matrix: O(num_classes²) memory only!
    ...
    with torch.inference_mode():
        for batch_idx, batch in enumerate(tqdm(val_loader, desc="Evaluating")):
            ...
            # FP16 inference - cuts memory nearly in half
            ...
            # Update confusion matrix on CPU (streaming - no accumulation on GPU)
            preds_flat = preds.view(-1).cpu()
            labels_flat = labels.view(-1).cpu()
            
            # Valid mask (ignore any invalid labels if present)
            ...
            # Immediate cleanup - critical!
            del images, labels, outputs, logits, preds, preds_flat, labels_flat
            
            # Periodic cache clear (every 100 batches, not every batch)
            if batch_idx % 100 == 0:
    ...
    for c in range(num_classes):
        if union[c] > 0:
            iou = (intersection[c].float() / union[c].float()).item()
            ious.append(iou)
...
def train(args):
    ...
    output_dir = os.path.join(args.output_dir, f'segformer_opt_{timestamp}')
    ...
    # Validation: batch_size=1 is critical for memory efficiency
    ...
    print(f"Train batch size: {batch_size}, Val batch size: 1")
    ...
    print("\n" + "=" * 60)
    print(f" OPTIMIZED Training: Epochs {start_epoch + 1} -> {args.epochs}")
    print("=" * 60)
    print("Features: FP16 eval, streaming mIoU, inference_mode")
    print("=" * 60)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # === CRITICAL: Clean up before evaluation ===
        gc.collect()
        torch.cuda.empty_cache()
        ...
        if (epoch + 1) % args.eval_interval == 0:
            ...
            # === CRITICAL: Clean up after evaluation ===
            gc.collect()
            torch.cuda.empty_cache()
